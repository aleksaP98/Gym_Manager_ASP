...
{
    public class EFGetTrainersQuery : IGetTrainersQuery
    {
        private readonly GymManagerContext _context;

        public EFGetTrainersQuery(GymManagerContext context)
        {
            _context = context;
        }

        public int Id => (int)UseCasesEnum.EFGetTrainers;

        public string Name => "Getting Trainers using EF";

        public PagedResponse<TrainerDto> Execute(TrainerSearch search)
        {
            ...
            return new PagedResponse<TrainerDto>
            {
                CurrentPage = search.Page,
                ItemsPerPage = search.PerPage,
                TotalCount = query.Count(),
                Items = query.Skip(skipCount).Take(search.PerPage).Select(x => new TrainerDto
                {
                    Address = x.Address,
